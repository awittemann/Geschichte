...
import type { StatistikSpeicher } from '@/lib/typen';
...
export async function PUT(request: Request) {
  ...
  let body: { statistik?: unknown };
  try {
    body = await request.json();
  } catch {
    return NextResponse.json({ fehler: 'Ungültiger JSON-Body' }, { status: 400 });
  }
  const s = body.statistik;
  if (!isObject(s) || !Array.isArray(s.tage)) {
    return NextResponse.json(
      { fehler: 'statistik fehlt oder falsches Format' },
      { status: 400 },
    );
  }
  const result = await mutiereDb((db) => {
    ...
    nutzer.statistik = s as unknown as StatistikSpeicher;
